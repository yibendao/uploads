...
class navmap_controller extends common
{
	...
	function index_action()
	{
		...
		if (trim($_GET['keyword'])){
			if($_GET['type']=='2'){
				$where.=" and `url` like '%".trim($_GET['keyword'])."%'";
			}else{
	...
	function add_action()
	{
		$type=$this->obj->DB_select_all("navmap"," 1 ","id,name");
	...
	function save_action(){
		if($_POST['submit']){
			...
			$this->cache_action();
			isset($nbid)?$this->ACT_layer_msg($msg."网站地图(ID:".$_POST['id'].")成功！",9,$_SERVER['HTTP_REFERER'],2,1):$this->ACT_layer_msg($msg."失败！",8,$_SERVER['HTTP_REFERER']);
		}
	}
	function del_action(){
		$this->check_token();
	...
	function nav_xianshi_action(){
	    $this->check_token();
	    $nid=$this->obj->DB_update_all("navmap","`".$_GET['type']."`='".intval($_GET['rec'])."'","`id`='".intval($_GET['id'])."'");
	    if ($_GET['type']=='display'){
	        $this->admin_log("导航是否显示(ID:".$_GET['id'].")设置成功！");
	    }else{
	        $this->admin_log("导航是否x新窗口打开(ID:".$_GET['id'].")设置成功！");
	    }
		$this->cache_action();
	    echo $nid?1:0;die;
	}
