...
class admin_keyword_controller extends common{
	...
	function ajax_action(){
		$keywordarr=array("1"=>"店铺招聘","2"=>"兼职","3"=>"职位","4"=>"公司","5"=>"简历","8"=>"微信搜索","13"=>"普工简历");
	...
	function recup_action(){
		...
		if($type=="bold")
		{
			$this->admin_log("对关键字 ".$row['name']." 是否加粗进行设置");
		}elseif($type=="tuijian"){
			$this->admin_log("对关键字 ".$row['name']." 是否推荐进行设置");
		}elseif($type=="check"){
			$this->admin_log("对关键字 ".$row['name']." 是否审核进行设置");
		}
		echo $nid?1:0;die;
	}
	function get_cache(){
