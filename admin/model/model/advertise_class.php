...
class advertise{
			...
			foreach($ad_list as $key=>$value){
				...
				if($start&&$start<$time && $end_type==1 && $value[is_check]=="1"){
					...
					if($ad_type=="word"){
						...
						if($value['target']==2){
							$show.= "\$ad_label['$value[class_id]']['ad_$id']['html']=\"<a href='$pic_src' target='_blank' rel='nofollow'><img src='$pic_url'  ".$height." ".$width." ".$alt."></a>\";\r\n";
						}else{
							$show.= "\$ad_label['$value[class_id]']['ad_$id']['html']=\"<a href='$pic_src' rel='nofollow'><img src='$pic_url' ".$height." ".$width." ".$alt." ></a>\";\r\n";
						}
					...
					}elseif($ad_type=="lianmeng"){
						
						$show.= "\$ad_label['$value[class_id]']['ad_$id']['html']=\"".str_replace('"','\\"',$lianmeng_url)."\";\r\n";
					}
	...
	function model_saveadd_action($post,$pic=NULL){
		...
		if($ad_type=="word")
		{
			...
			$value.=",`pic_url`='$pic_url',`pic_src`='$pic_src',`pic_content`='$pic_content',`pic_width`='$pic_width',`pic_height`='$pic_height'";
			$nid = $this->obj->DB_insert_once("ad",$value);
		}elseif($ad_type=="flash"){
			if($pic!=""){
				$flash_url = $pic;
			...
			$value.=",`lianmeng_url`='$lianmeng_url'";
