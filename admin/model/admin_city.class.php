	...
	function upp_action(){
		if($_POST['id']!="" || $_POST['addcityname_0'])
		{
			if($_POST['updateall'])
			{
				...
				$this->cache_action();
				$this->ACT_layer_msg("区域(ID:".$value.")修改成功！",9,$_SERVER['HTTP_REFERER'],2,1);
			}
			if($_POST['id']){

				$city_arr = $this->obj->DB_select_all("city_class","`id` IN (".$_POST['id'].") OR `keyid` IN (".$_POST['id'].")");
				...
				$this->obj->DB_delete_all("city_class",$where," ");
				$this->cache_action();
		...
		if( $kid && $type ){
			...
			if(is_array($adcity)){
				foreach($adcity as $key=>$value){
					...
					$html.="<td class=\"ud\"><input type=\"text\" name=\"citysort_".$value["id"]."\" id=\"citysort_".$value["id"]."\" value=\"".$value[sort]."\" class=\"input-text citysort\" size=\"3\"></td>";
