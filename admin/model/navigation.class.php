...
class navigation_controller extends common{
	...
	function deltype_action(){
	   ...
	   if(isset($_GET['id'])){
			...
			if(is_array($rows)){
				...
				$this->obj->DB_update_all("description","`is_menu`='0'","`id` in (".@implode(",",$desc).")");
				$this->obj->DB_update_all("news_group","`is_menu`='0'","`id` in (".@implode(",",$news).")");
			}
	...
	function nav_xianshi_action(){
	    $this->check_token();
	    $nid=$this->obj->DB_update_all("navigation","`".$_GET['type']."`='".intval($_GET['rec'])."'","`id`='".intval($_GET['id'])."'");
	    if ($_GET['type']=='display'){
	        $this->admin_log("导航是否显示(ID:".$_GET['id'].")设置成功！");
	    }else{
	        $this->admin_log("导航是否新窗口打开(ID:".$_GET['id'].")设置成功！");
	    }
		$this->cache_action();
	    echo $nid?1:0;die;
	}
