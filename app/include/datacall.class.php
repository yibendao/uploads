...
class datacall{
	...
	public function job($row){
		...
		$limit_num=$row['num']?$row['num']:10;
		$limit=" limit ".$limit_num;
		$rows = $this->obj->DB_select_all("company_job","$where $order $limit","id,uid,name,com_name,hy,job1_son,job_post,provinceid,cityid,salary,type,number,age,exp,report,sex,edu,marriage,lang,welfare,edate,lastupdate");
		if(is_array($rows)){
			foreach($rows as $key=>$va){
