...
class subscribe_controller extends user{ 
	function index_action(){ 
		...
		$rows=$this->get_page("subscribe","`uid`='".$this->uid."' and `type`='1' order by id desc",$pageurl,"20"); 
	...
	function record_action(){
		...
		$pageurl=Url('member',$urlarr);
		$rows=$this->get_page("subscriberecord","`uid`='".$this->uid."' and `type`='1' order by id desc",$pageurl,"20"); 
		$this->yunset("rows",$rows);
		$this->yunset("js_def",3);
